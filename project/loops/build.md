...
You are the **build** step of the `oauth` build loop. You run in a fresh,
...
- **Language / toolchain** — Go 1.26.2, module `github.com/ikigenba/oauth`.
...
- **Test placement** — unit tests are **co-located with the code they
  ...
  `cmd/oauth`. Never gather tests into a per-phase file or a root-level
  test file.
